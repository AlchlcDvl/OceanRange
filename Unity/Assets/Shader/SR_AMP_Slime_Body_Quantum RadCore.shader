Shader "SR/AMP/Slime/Body/Quantum RadCore" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5.xy = tmp1.xx * float2(0.4, 0.4) + float2(0.4, -0.1);
                tmp1.x = 1.0 / tmp1.w;
                tmp1.x = tmp1.x - 0.2;
                ...
                tmp6.y = -tmp1.w * _ProjectionParams.x + tmp0.w;
                tmp6.x = inp.texcoord4.x;
                tmp6.xy = tmp6.xy / tmp1.xx;
                tmp6.z = tmp6.x * 1.78;
                ...
                tmp1.w = tmp1.w * 0.9;
                tmp1.w = exp(tmp1.w);
                tmp1.zw = tmp1.zw * float2(5.688889, 3.0);
                tmp1.z = floor(tmp1.z);
                ...
                tmp1.xyz = tmp0.xyz * float3(1.5, 1.5, 1.5);
                ...
                tmp2.x = -tmp5.y * 2.0 + 1.0;
                tmp2.y = tmp5.x > 0.5;
                tmp1.w = -tmp2.x * tmp1.w + 1.0;
                tmp0.w = saturate(tmp2.y ? tmp1.w : tmp0.w);
                tmp2.xyz = _GlowTop.xyz * float3(0.333, 0.333, 0.333);
                tmp3.xyz = tmp0.www * tmp2.xyz;
                tmp3.xyz = _GlowTop.xyz * float3(0.667, 0.667, 0.667) + tmp3.xyz;
                tmp4.xyz = tmp0.www * _GlowTop.xyz;
                tmp2.xyz = tmp4.xyz * float3(0.334, 0.334, 0.334) + tmp2.xyz;
                ...
                tmp3.xyz = max(tmp1.xyz, tmp3.xyz);
