Shader "SR/AMP/Slime/Body/Galaxy Stripe" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xy = _StripeUV1.xx * tmp0.zw + inp.texcoord.xy;
                tmp0.zw = _Stripe2UV1.xx * tmp0.zw + inp.texcoord.xy;
                tmp3.x = 0.0;
                tmp3.y = _StripeSpeed * _Time.y;
                tmp2.xy = tmp2.xy + tmp3.xy;
                tmp0.zw = tmp0.zw + tmp3.xy;
                tmp3 = tex2D(_Stripe2Texture, tmp0.zw);
                tmp0.z = tmp3.x - _StripeRemap2.x;
                tmp2 = tex2D(_StripeTexture, tmp2.xy);
                tmp0.w = tmp2.x - _StripeRemap1.x;
                tmp2.xy = _StripeRemap1.wy - _StripeRemap1.zx;
                tmp0.w = tmp0.w * tmp2.x;
                tmp0.w = tmp0.w / tmp2.y;
                tmp0.w = saturate(tmp0.w + _StripeRemap1.z);
                tmp2.xy = _StripeRemap2.wy - _StripeRemap2.zx;
                tmp0.z = tmp0.z * tmp2.x;
                tmp0.z = tmp0.z / tmp2.y;
                tmp0.z = saturate(tmp0.z + _StripeRemap2.z);
                ...
                tmp0.w = tmp2.x * tmp0.w + -inp.texcoord.y;
                tmp0.w = _LightingUVHorizontalAdjust * tmp0.w + inp.texcoord.y;
                tmp0.w = tmp0.w - 0.5;
                tmp2.x = _LightingUVContribution * tmp0.w + 0.5;
                ...
                tmp2.z = rsqrt(tmp2.z);
                tmp2.w = tmp3.y * tmp2.z + 1.0;
                tmp4.xyz = tmp2.zzz * tmp3.xyz;
                tmp2.z = saturate(tmp2.w * 0.75 + -0.5);
                tmp2.w = 1.0 - tmp2.z;
                tmp2.x = dot(tmp2.xy, tmp2.xy);
                tmp0.w = -tmp0.w * tmp2.w + 1.0;
                tmp0.w = saturate(tmp2.y ? tmp0.w : tmp2.x);
                tmp0.w = tmp0.w * 0.85;
                ...
                tmp1.xyz = tmp1.xyz * tmp2.xxx + float3(0.0, 1.0, 0.0);
                ...
                tmp1.y = tmp1.x * 0.625;
                tmp0.z = saturate(tmp0.z * tmp0.w + tmp1.y);
                tmp0.w = dot(tmp3.xyz, tmp2.xyz);
                tmp2 = texCUBE(_Stars, tmp2.yzw);
                tmp1.y = saturate(tmp2.x * 5.0);
                ...
                tmp0.xzw = tmp0.xxx * tmp3.xyz + tmp4.xyz;
                tmp1.yzw = tmp1.yzw - tmp0.xzw;
                tmp0.xyz = tmp0.yyy * tmp1.yzw + tmp0.xzw;
                tmp1.xyz = tmp1.xxx * float3(0.625, 0.625, 0.625) + tmp0.xyz;
