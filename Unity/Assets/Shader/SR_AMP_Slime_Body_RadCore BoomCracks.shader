Shader "SR/AMP/Slime/Body/RadCore BoomCracks" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.z = tmp1.z + 1.0;
                tmp1.w = 1.0 - tmp1.z;
                tmp2.w = tmp1.x * 3.0 + -_Time.y;
                tmp1.x = tmp1.x * -1.5 + 1.5;
                tmp2.w = frac(tmp2.w);
                tmp2.w = tmp2.w * 2.0 + -1.0;
                tmp2.w = abs(tmp2.w) * _RadExoticCore;
                tmp3.xy = tmp2.ww * float2(0.4, 0.4) + float2(-0.1, 0.4);
                ...
                tmp3.xyz = _GlowTop.xyz * float3(0.333, 0.333, 0.333);
                tmp4.xyz = tmp1.zzz * tmp3.xyz;
                tmp4.xyz = _GlowTop.xyz * float3(0.667, 0.667, 0.667) + tmp4.xyz;
                tmp5.xyz = tmp1.zzz * _GlowTop.xyz;
                tmp3.xyz = tmp5.xyz * float3(0.334, 0.334, 0.334) + tmp3.xyz;
                tmp4.xyz = tmp4.xyz - tmp3.xyz;
                tmp3.xyz = tmp1.zzz * tmp4.xyz + tmp3.xyz;
                ...
                tmp4.xyz = max(tmp2.xyz, tmp4.xyz);
                tmp3.xyz = tmp3.xyz - tmp4.xyz;
                tmp3.xyz = tmp1.zzz * tmp3.xyz + tmp4.xyz;
                tmp0.xyz = -tmp0.xyz * float3(1.5, 1.5, 1.5) + tmp3.xyz;
                tmp0.xyz = _GlowTop.www * tmp0.xyz + tmp2.xyz;
                tmp1.z = dot(tmp0.xyz, float3(0.299, 0.587, 0.114));
                tmp2.xyz = tmp1.zzz - tmp0.xyz;
                ...
                tmp1.zw = tmp2.ww * float2(0.06, -3.0) + tmp1.zw;
                tmp4 = tex2D(_CrackNoise, tmp1.zw);
                tmp1.z = tmp4.x * 2.0 + -1.0;
                tmp1.z = tmp1.z * _CrackAmount;
                tmp3.xyz = tmp1.zzz * float3(0.03, 0.03, 0.03) + tmp3.xyz;
                tmp3 = texCUBE(_Cracks, tmp3.xyz);
                tmp1.z = tmp3.x * -0.25 + 1.0;
                tmp1.x = tmp1.x * tmp3.x;
                tmp1.x = tmp1.x * _CrackAmount;
                tmp2.xyz = tmp2.xyz * tmp1.zzz;
                tmp2.xyz = tmp2.xyz * float3(0.75, 0.75, 0.75) + -tmp0.xyz;
                tmp2.xyw = _Char.xxx * tmp2.yzx + tmp0.yzx;
                ...
                tmp2.yzw = abs(tmp0.yyy) + float3(1.0, 0.6666667, 0.3333333);
                ...
                tmp0.xyz = tmp0.xxx * tmp2.yzw + float3(1.0, 1.0, 1.0);
                tmp3.x = tmp1.x * 40.0;
                tmp1.xz = tmp1.xx * float2(10.0, 10.0) + float2(-0.8, -0.333);
                tmp1.xz = saturate(tmp1.xz * float2(5.0, 2.994012));
                tmp3.x = saturate(tmp3.x);
                tmp1.w = tmp3.x * -2.0 + 3.0;
                tmp2.y = tmp3.x * tmp3.x;
                tmp1.w = tmp1.w * tmp2.y;
                tmp1.w = tmp1.w * -0.05 + 1.0;
                tmp1.w = tmp1.w * tmp2.x;
                tmp2.xyz = _CrackAmount.xxx * float3(-0.4, -1.3, -0.25) + float3(1.0, 2.0, 1.0);
                tmp1.w = tmp1.w * tmp2.z;
                tmp2.z = inp.texcoord.y * 0.5 + 0.5;
                tmp1.y = tmp1.y * tmp2.z + -tmp2.x;
                ...
                tmp2.xy = tmp1.xz * float2(-2.0, -2.0) + float2(3.0, 3.0);
                tmp1.xz = tmp1.xz * tmp1.xz;
                tmp1.xz = tmp1.xz * tmp2.xy;
                tmp1.x = tmp1.z * 0.5 + tmp1.x;
                tmp1.z = 1.0 - tmp1.x;
                tmp2.x = 1.0 - tmp4.x;
                tmp1.z = tmp1.z / tmp2.x;
                tmp1.z = saturate(1.0 - tmp1.z);
                tmp1.z = tmp1.x * 0.667 + tmp1.z;
                ...
                tmp6.xyz = tmp1.zzz * tmp3.yzw + tmp5.xyz;
                tmp1.z = tmp0.w * _CrackAmount;
                tmp3.yzw = tmp1.zzz * tmp3.yzw + tmp5.xyz;
                tmp3.yzw = _CrackAmount.xxx * tmp0.www + tmp3.yzw;
                tmp3.yzw = saturate(tmp3.yzw - float3(1.0, 1.0, 1.0));
                tmp3.yzw = tmp6.xyz * tmp1.xxx + tmp3.yzw;
                tmp3.yzw = tmp3.yzw + tmp3.yzw;
                tmp5.xyz = tmp3.xxx * tmp4.yzw + -tmp2.yzw;
                tmp4.yzw = tmp3.xxx * tmp4.yzw;
                tmp2.xyz = _CrackAmount.xxx * tmp5.xyz + tmp2.yzw;
                ...
                tmp1.xyz = tmp2.xyz * tmp1.yyy + tmp3.yzw;
