Shader "SR/AMP/Slime/Body/PhosCore Galaxy" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.yz = inp.texcoord4.xy / tmp1.xx;
                tmp2.xy = tmp1.yz - float2(0.5, 0.5);
                tmp3 = tmp1.yzyz * float4(1.1, 1.1, 0.8, 0.8);
                tmp3 = _Time * float4(0.134, 0.066, -0.2, -0.1) + tmp3;
                tmp2.z = tmp0.y * tmp2.y;
                tmp1.yz = tmp2.xz * float2(2.0, 2.0) + float2(1.0, 1.0);
                tmp0.yz = tmp1.yz * float2(0.5, 0.5) + -tmp0.zw;
                ...
                tmp7.y = tmp4.x;
                tmp7.x = -tmp2.x;
                tmp4.y = dot(tmp0.xy, tmp7.xy);
                tmp4.x = dot(tmp0.xy, tmp7.xy);
                tmp0.xy = tmp4.xy + float2(0.5, 0.5);
                tmp0 = tex2D(_SpiralGalaxy, tmp0.xy);
                tmp0.x = tmp0.z + 0.5;
                tmp4 = tex2D(_SpiralGalaxy, tmp3.xy);
                tmp3 = tex2D(_SpiralGalaxy, tmp3.zw);
                tmp0.y = tmp3.y * tmp4.y;
                tmp0.z = tmp0.y * 0.5 + 0.5;
                tmp0.x = tmp0.x * tmp0.z;
                tmp3.x = inp.texcoord1.w;
                tmp3.z = inp.texcoord3.w;
                tmp0.zw = tmp3.xz - unity_ObjectToWorld._m03_m23;
                ...
                tmp2.y = tmp6.x;
                tmp2.x = -tmp5.x;
                tmp4.y = dot(tmp0.xy, tmp2.xy);
                tmp4.x = dot(tmp0.xy, tmp2.xy);
                ...
                tmp2.w = 1.0 - tmp1.w;
                tmp0.w = dot(tmp1.xy, tmp0.xy);
                tmp0.z = -tmp0.z * tmp2.w + 1.0;
                tmp0.z = saturate(tmp1.z ? tmp0.z : tmp0.w);
                tmp0.z = tmp0.z * 0.85;
                ...
                tmp3.xyz = tmp3.xyz * tmp0.www + float3(0.0, 1.0, 0.0);
                tmp0.w = dot(tmp4.xyz, tmp5.xyz);
                tmp0.w = 1.0 - tmp0.w;
                tmp0.z = tmp0.w * tmp0.w + tmp0.z;
                tmp0.w = log(tmp0.w);
                tmp0.xw = tmp0.xw * float2(4.0, 0.9);
                tmp0.w = exp(tmp0.w);
                tmp0.w = tmp0.w * 3.0;
                ...
                tmp0.z = tmp1.y * 0.625 + tmp0.z;
                tmp0.z = tmp0.z + 0.15;
                tmp1.z = saturate(tmp0.z);
                tmp0.z = _Alpha * 0.5 + tmp0.z;
                tmp0.z = saturate(tmp0.z + 0.5);
                tmp1.w = dot(tmp2.xyz, tmp5.xyz);
                tmp2 = texCUBE(_Stars, tmp5.xyz);
                tmp2.x = saturate(tmp2.x * 5.0);
                tmp0.y = saturate(dot(tmp2.xy, tmp0.xy));
                tmp1.w = 1.0 - tmp1.w;
                tmp1.z = tmp1.z - tmp1.w;
                tmp1.z = _BodyLightingContribution * tmp1.z + tmp1.w;
                tmp1.z = tmp1.z * -0.75 + 1.0;
                ...
                tmp4.xyz = tmp1.yyy * float3(0.625, 0.625, 0.625) + tmp2.xyz;
                tmp4.xyz = -tmp3.xyz * tmp2.xyz + tmp4.xyz;
                tmp2.xyz = tmp2.xyz * tmp3.xyz;
                tmp2.xyz = tmp4.xyz * float3(0.8, 0.8, 0.8) + tmp2.xyz;
                tmp0.x = tmp1.x * 0.5;
                tmp0.y = -tmp1.x * 0.5 + inp.texcoord4.y;
                tmp3.yw = -tmp0.yy * _ProjectionParams.xx + tmp0.xx;
                ...
                tmp0.xy = tmp1.ww * tmp0.xy + tmp3.xy;
                tmp1.x = tmp1.w * 3.0 + -_Time.y;
                tmp1.x = frac(tmp1.x);
                tmp1.x = tmp1.x * 2.0 + -1.0;
                tmp1.x = abs(tmp1.x) * _RadExoticCore;
                tmp1.xy = tmp1.xx * float2(0.4, 0.4) + float2(0.4, -0.1);
                tmp3 = tex2D(_RefractionSlime, tmp0.xy);
                tmp2.xyz = tmp2.xyz - tmp3.xyz;
                tmp0.xyz = tmp0.zzz * tmp2.xyz + tmp3.xyz;
                tmp1.zw = _RadExoticCore.xx * float2(1.46, 0.11) + float2(1.0, -1.0);
