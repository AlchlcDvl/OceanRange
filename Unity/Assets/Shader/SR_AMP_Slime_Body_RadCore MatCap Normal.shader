Shader "SR/AMP/Slime/Body/RadCore MatCap Normal" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = tmp2.xyz * tmp0.yyy + float3(0.0, 1.0, 0.0);
                ...
                tmp0.xyz = tmp0.yyy * float3(0.625, 0.625, 0.625) + tmp4.xyz;
                tmp0.xyz = -tmp5.xyz * tmp4.xyz + tmp0.xyz;
                tmp4.xyz = tmp4.xyz * tmp5.xyz;
                tmp0.xyz = tmp0.xyz * float3(0.8, 0.8, 0.8) + tmp4.xyz;
                ...
                tmp2.z = tmp3.z * 1.0;
                tmp4.xyz = tmp1.zzz * unity_MatrixV._m01_m11_m21;
                tmp4.xyz = unity_MatrixV._m00_m10_m20 * tmp1.yyy + tmp4.xyz;
                tmp1.yzw = unity_MatrixV._m02_m12_m22 * tmp1.www + tmp4.xyz;
                tmp4.z = tmp1.w * tmp2.z;
                tmp4.xy = tmp3.xy * float2(-1.0, -1.0) + tmp1.yz;
                tmp1.y = dot(tmp4.xyz, tmp4.xyz);
                tmp1.y = rsqrt(tmp1.y);
                tmp1.yz = tmp1.yy * tmp4.xy;
                tmp1.yz = tmp1.yz * float2(0.5, 0.5) + float2(0.5, 0.5);
                tmp3 = tex2D(_CubemapOverride, tmp1.yz);
                tmp1.yzw = tmp3.xyz > float3(0.5, 0.5, 0.5);
                ...
                tmp3.xyz = -tmp3.xyz * float3(0.75, 0.75, 0.75) + float3(1.0, 1.0, 1.0);
                tmp1.yzw = saturate(tmp1.yzw ? tmp3.xyz : tmp4.xyz);
                tmp1.yzw = -tmp0.xyz * float3(1.5, 1.5, 1.5) + tmp1.yzw;
                tmp0.xyz = tmp0.xyz * float3(1.5, 1.5, 1.5);
                tmp1.yzw = tmp0.www * tmp1.yzw;
                tmp0.xyz = _UseOverride.xxx * tmp1.yzw + tmp0.xyz;
                tmp1.yz = _RadExoticCore.xx * float2(1.46, 0.11) + float2(1.0, -1.0);
                tmp0.w = tmp1.z * tmp1.x;
