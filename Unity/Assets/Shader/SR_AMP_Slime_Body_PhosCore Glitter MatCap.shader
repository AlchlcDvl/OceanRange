	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5.xyz = -tmp5.xyz * tmp1.yyy + float3(1.0, 1.0, 1.0);
                tmp6.xyz = tmp4.xyz + tmp4.xyz;
                tmp4.xyz = tmp4.xyz > float3(0.5, 0.5, 0.5);
                tmp1.xyz = tmp1.xxx * tmp6.xyz;
                tmp1.xyz = saturate(tmp4.xyz ? tmp5.xyz : tmp1.xyz);
                tmp4.xy = inp.texcoord.zw - inp.texcoord.xy;
                tmp4.xy = _OverrideAlphaUV1.xx * tmp4.xy + inp.texcoord.xy;
                tmp4 = tex2D(_CubemapOverride, tmp4.xy);
                tmp2.w = tmp4.w * _OverrideBlend;
                ...
                tmp4.xyz = tmp0.zzz * tmp4.xyz + tmp6.xyz;
                ...
                tmp6.xyz = tmp0.zzz * tmp6.xyz + tmp7.xyz;
                tmp0.z = saturate(tmp0.z * 2.0 + -1.0);
                tmp4.xyz = tmp4.xyz - tmp6.xyz;
                tmp4.xyz = tmp0.zzz * tmp4.xyz + tmp6.xyz;
                tmp6.xyz = tmp0.yyy * float3(0.625, 0.625, 0.625) + tmp4.xyz;
                tmp6.xyz = -tmp5.xyz * tmp4.xyz + tmp6.xyz;
                tmp4.xyz = tmp4.xyz * tmp5.xyz;
                tmp4.xyz = tmp6.xyz * float3(0.8, 0.8, 0.8) + tmp4.xyz;
                tmp4.xyz = float3(1.0, 1.0, 1.0) - tmp4.xyz;
                tmp1.xyz = saturate(-tmp4.xyz * tmp1.xyz + float3(1.0, 1.0, 1.0));
                tmp0.y = inp.texcoord4.w + 0.0;
                tmp0.z = tmp0.y * 0.5;
                tmp2.w = -tmp0.y * 0.5 + inp.texcoord4.y;
                tmp4.yw = -tmp2.ww * _ProjectionParams.xx + tmp0.zz;
                tmp4.xz = inp.texcoord4.xx;
                tmp4 = tmp4 / tmp0.yyyy;
                tmp4 = tmp4 - float4(0.5, 0.5, 0.5, 0.5);
                tmp4 = tmp4 * float4(0.5, 0.5, 1.5, 1.5) + float4(0.5, 0.5, 0.5, 0.5);
                tmp0.yz = tmp4.zw - tmp4.xy;
                tmp0.yz = tmp0.ww * tmp0.yz + tmp4.xy;
                tmp0.w = tmp0.w * 3.0 + -_Time.y;
                tmp0.w = frac(tmp0.w);
                tmp0.w = tmp0.w * 2.0 + -1.0;
                tmp0.w = abs(tmp0.w) * _RadExoticCore;
                tmp4.xy = tmp0.ww * float2(0.4, 0.4) + float2(-0.1, 0.4);
                tmp5 = tex2D(_RefractionSlime, tmp0.yz);
