Shader "SR/AMP/Slime/Body/RadCore Galaxy" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.zw = tmp2.xz * float2(2.0, 2.0) + float2(1.0, 1.0);
                tmp0.yz = tmp1.zw * float2(0.5, 0.5) + -tmp0.zw;
                ...
                tmp0.x = tmp0.z + 0.5;
                tmp3 = _Time * float4(0.134, 0.066, -0.2, -0.1);
                tmp1 = tmp1.xyxy * float4(1.1, 1.1, 0.8, 0.8) + tmp3;
                ...
                tmp1.yz = inp.texcoord.xy * _Noise_ST.xy + _Noise_ST.zw;
                tmp4.xy = _Time.yy * float2(-0.1, -0.1) + tmp1.yz;
                tmp1.yz = _Time.yy * float2(0.1, -0.25) + tmp1.yz;
                tmp5 = tex2D(_Noise, tmp1.yz);
                tmp4 = tex2D(_Noise, tmp4.xy);
                tmp1.y = 1.0 - tmp4.x;
                tmp1.y = tmp1.y / tmp5.x;
                ...
                tmp0.yzw = tmp0.xxx * tmp3.xyz + tmp0.yzw;
