Shader "SR/AMP/Slime/Body/RadCore Normal" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.yz = tmp0.ww * float2(0.4, 0.4) + float2(0.4, -0.1);
                tmp0.z = tmp0.z * tmp1.x;
                tmp0.w = log(tmp1.x);
                tmp0.w = tmp0.w * 0.9;
                tmp0.w = exp(tmp0.w);
                ...
                tmp0.xyz = tmp0.xxx * float3(0.625, 0.625, 0.625) + tmp2.xyz;
                ...
                tmp1.xw = _RadExoticCore.xx * float2(1.46, 0.11) + float2(1.0, -1.0);
