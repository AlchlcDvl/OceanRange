Shader "SR/AMP/Slime/Body/Glitter MatCap Stripe" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5.xyz = unity_MatrixV._m02_m12_m22 * tmp3.zzz + tmp5.xyz;
                tmp6.z = tmp1.w * tmp5.z;
                tmp6.xy = tmp2.xy * float2(-1.0, -1.0) + tmp5.xy;
                tmp1.w = dot(tmp6.xyz, tmp6.xyz);
                tmp1.w = rsqrt(tmp1.w);
                tmp2.xy = tmp1.ww * tmp6.xy;
                ...
                tmp5.xyz = -tmp5.xyz * tmp1.zzz + float3(1.0, 1.0, 1.0);
                tmp6.xyz = tmp0.www * tmp2.xyz;
                tmp2.xyz = tmp2.xyz > float3(0.5, 0.5, 0.5);
                tmp6.xyz = tmp6.xyz + tmp6.xyz;
                tmp2.xyz = saturate(tmp2.xyz ? tmp5.xyz : tmp6.xyz);
                tmp1.xzw = tmp1.xxx * tmp0.xxx + tmp2.xyz;
                tmp1.xzw = tmp1.xzw - float3(1.0, 1.0, 1.0);
                tmp1.xzw = max(tmp1.xzw, float3(0.0, 0.0, 0.0));
                tmp0.xzw = tmp0.zzz * tmp1.xzw;
                tmp0.xzw = -_UseOverride.xxx * tmp0.xzw + float3(1.0, 1.0, 1.0);
                ...
                tmp1.xzw = tmp0.yyy * tmp1.xzw + tmp5.xyz;
                ...
                tmp1.xzw = tmp1.xzw - tmp5.xyz;
                tmp1.xzw = tmp0.yyy * tmp1.xzw + tmp5.xyz;
                tmp5.xyz = tmp1.yyy * float3(0.625, 0.625, 0.625) + tmp1.xzw;
                tmp5.xyz = -tmp2.xyz * tmp1.xzw + tmp5.xyz;
                tmp1.xyz = tmp1.xzw * tmp2.xyz;
                tmp1.xyz = tmp5.xyz * float3(0.8, 0.8, 0.8) + tmp1.xyz;
                tmp1.xyz = float3(1.0, 1.0, 1.0) - tmp1.xyz;
                tmp0.xyz = saturate(-tmp1.xyz * tmp0.xzw + float3(1.0, 1.0, 1.0));
                tmp0.w = dot(tmp4.xyz, tmp4.xyz);
                tmp0.w = rsqrt(tmp0.w);
                tmp1.xyz = tmp0.www * tmp4.xyz;
                tmp2.xyz = tmp3.yyy * unity_WorldToObject._m01_m11_m21;
                tmp2.xyz = unity_WorldToObject._m00_m10_m20 * tmp3.xxx + tmp2.xyz;
                tmp2.xyz = unity_WorldToObject._m02_m12_m22 * tmp3.zzz + tmp2.xyz;
