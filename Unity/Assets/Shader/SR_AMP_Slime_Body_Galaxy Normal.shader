	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = _GalaxyLargoScale.xxx * float3(-0.5, -0.15, -0.133) + float3(1.0, 0.4, 0.8);
                ...
                tmp0.z = tmp1.w * tmp0.z + -inp.texcoord.y;
                tmp0.z = _LightingUVHorizontalAdjust * tmp0.z + inp.texcoord.y;
                tmp0.z = tmp0.z - 0.5;
                tmp1.w = tmp0.z * _LightingUVContribution;
                ...
                tmp2.xy = _NormalUV1.xx * tmp2.xy + inp.texcoord.xy;
                ...
                tmp0.w = rsqrt(tmp0.w);
                tmp2.x = tmp3.y * tmp0.w + 1.0;
                tmp2.yzw = tmp0.www * tmp3.xyz;
                tmp0.w = saturate(tmp2.x * 0.75 + -0.5);
                tmp2.x = 1.0 - tmp0.w;
                tmp0.w = dot(tmp0.xy, tmp0.xy);
                tmp0.z = tmp0.z > 0.5;
                tmp1.w = -tmp1.w * tmp2.x + 1.0;
                tmp0.z = saturate(tmp0.z ? tmp1.w : tmp0.w);
                tmp0.z = tmp0.z * 0.85;
                ...
                tmp1 = texCUBE(_Stars, tmp4.xyz);
                tmp0.z = saturate(tmp1.x * 5.0);
