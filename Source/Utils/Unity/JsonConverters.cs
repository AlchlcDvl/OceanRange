...
public abstract class OceanJsonConverter : JsonConverter
{
    ...
    /// <inheritdoc/>
    public override sealed object ReadJson(JsonReader reader, Type objectType, [AllowNull] object _1, JsonSerializer serializer)
    {
    ...
    /// <inheritdoc/>
    public override sealed void WriteJson(JsonWriter writer, [AllowNull] object value, JsonSerializer serializer)
    {
        if (value == null)
            writer.WriteNull();
    ...
    /// <summary>
    /// A converter's implementation of reading from json with access to the serializer.
    /// </summary>
    /// <param name="reader">The json reader.</param>
    /// <param name="objectType">The type of the object (used for non generics).</param>
    /// <param name="serializer">The serializer used for nested conversions.</param>
    /// <returns>The deserialised value.</returns>
    protected abstract object Read(JsonReader reader, Type objectType, JsonSerializer serializer);
    ...
    /// <summary>
    /// Writes the JSON representation of the object with access to the serializer.
    /// </summary>
    /// <param name="writer">The JsonWriter to write to.</param>
    /// <param name="value">The value to write.</param>
    /// <param name="serializer">The serializer used for nested conversions.</param>
    protected virtual void Write(JsonWriter writer, [AllowNull] object value, JsonSerializer serializer) => throw new InvalidOperationException();
}
...
public abstract class OceanJsonConverter<T> : OceanJsonConverter
{
    ...
    /// <inheritdoc/>
    protected override sealed object Read(JsonReader reader, Type _, JsonSerializer serializer) => Read(reader, serializer);

    /// <inheritdoc/>
    protected override sealed void Write(JsonWriter writer, [AllowNull] object value, JsonSerializer serializer)
    {
        if (value is T tValue)
            Write(writer, tValue, serializer);
        else
            throw new JsonSerializationException("Received unknown value: " + value);
    }
    ...
    protected abstract T Read(JsonReader reader, JsonSerializer serializer);
    ...
    protected virtual void Write(JsonWriter writer, [AllowNull] T value, JsonSerializer serializer) => throw new InvalidOperationException();
}

/// <summary>
/// Generic converter for lists of SerializableKeyValuePair of strings and custom objects to map-like JSON objects.
/// </summary>
/// <typeparam name="TValue">Value type.</typeparam>
/// <typeparam name="TPair">Concrete pair type.</typeparam>
public abstract class KeyValueListJsonConverter<TValue, TPair> : OceanJsonConverter<List<TPair>>
    where TPair : StringDictEntry<TValue>, new()
{
    protected override bool CustomSerialisation => true;

    protected override List<TPair> Read(JsonReader reader, JsonSerializer serializer)
    {
        if (reader.TokenType != JsonToken.StartObject)
            throw new JsonSerializationException($"Expected object for dictionary-like list, got {reader.TokenType}.");

        var obj = JObject.Load(reader);
        var result = new List<TPair>(obj.Count);
    ...
    protected override void Write(JsonWriter writer, List<TPair> value, JsonSerializer serializer)
    {
...
// Concrete converters for the specific pair list types we use
public sealed class StringColorPairListConverter : KeyValueListJsonConverter<Color, StringColorDictEntry>;

public sealed class StringStringPairListConverter : KeyValueListJsonConverter<string, StringStringDictEntry>;

public sealed class StringStringArrayPairListConverter : KeyValueListJsonConverter<string[], StringStringArrayDictEntry>;

public sealed class StringVector3ArrayPairListConverter : KeyValueListJsonConverter<Vector3[], StringVector3ArrayDictEntry>;

public sealed class StringZoneRequirementPairListConverter : KeyValueListJsonConverter<ZoneRequirementData, StringZoneRequirementDataDictEntry>;

// Nested list converters
public sealed class StringToStringPairListConverter : KeyValueListJsonConverter<List<StringStringDictEntry>, NestedStringStringDictEntryLayer1>;

public sealed class StringToStringPairListPairListConverter : KeyValueListJsonConverter<List<NestedStringStringDictEntryLayer1>, NestedStringStringDictEntryLayer2>;

public sealed class StringToStringVector3ArrayPairListConverter : KeyValueListJsonConverter<List<StringVector3ArrayDictEntry>, NestedStringVector3ArrayDictEntry>;
    ...
    where TComponent : struct // The type that make up the value's components
{
    ...
    /// <inheritdoc/>
    protected override sealed TValue Read(JsonReader reader, JsonSerializer _)
    {
...
public sealed class OrientationConverter : OceanJsonConverter<Orientation>
{
    ...
    protected override Orientation Read(JsonReader reader, JsonSerializer _)
    {
        var valString = reader.Value?.ToString() ?? "null";
...
//     protected override object Read(JsonReader reader, Type objectType, JsonSerializer _)
//     {
...
//     /// <inheritdoc/>
//     protected override void Write(JsonWriter writer, [AllowNull] object value, JsonSerializer _)
//     {
//         if (value == null)
//         {
//             writer.WriteNull();
...
//     /// <inheritdoc/>
//     protected override Type Read(JsonReader reader, JsonSerializer _)
//     {
//         if (reader.TokenType != JsonToken.String)
...
public sealed class OceanJsonContractResolver : DefaultContractResolver
{
    private const BindingFlags SerializeFlags = BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy;

    public OceanJsonContractResolver() : base() => NamingStrategy = new CamelCaseNamingStrategy();

    protected override List<MemberInfo> GetSerializableMembers(Type objectType)
    {
        var members = new HashSet<MemberInfo>();

        foreach (var field in objectType.GetFields(SerializeFlags))
        {
            if (!field.IsLiteral && field.GetCustomAttribute<JsonPropertyAttribute>() != null)
                members.Add(field);
        }

        foreach (var property in objectType.GetProperties(SerializeFlags))
        {
            if (property.CanWrite && property.CanRead && property.GetCustomAttribute<JsonPropertyAttribute>() != null)
